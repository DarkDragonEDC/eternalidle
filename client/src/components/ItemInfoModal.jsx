...
const ItemInfoModal = ({ item: rawItem, onClose }) => {
    ...
    const getItemDescription = (itm) => {
        ...
        if (itm.type === 'HELMET' || itm.type === 'ARMOR' || itm.type === 'BOOTS' || itm.type === 'GLOVES') return "Defensive equipment. Increases your Health and Defense.";
        if (itm.type === 'OFF_HAND') return itm.id?.includes('SHEATH') ? "Secondary defensive equipment." : "Secondary equipment. Offers various bonuses.";
        if (itm.type === 'CAPE') return "Special cape. Offers passive bonuses and global efficiency.";
        if (itm.type && itm.type.startsWith('TOOL')) return "Gathering tool. Required to gather higher TIER resources.";
    ...
    return createPortal(
        <AnimatePresence>
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 30000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={handleBackdropClick}>
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
                    ...
                    <div style={{ padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto', flex: 1, paddingRight: '16px', margin: '0 4px 20px 0' }} className="custom-scrollbar">
                        ...
                        {/* Description Section */}
                        <div style={{ background: 'var(--accent-soft)', padding: '12px', borderRadius: '14px', border: '1px solid var(--border)', textAlign: 'center' }}>
                        ...
                        {(() => {
                            ...
                            return (
                                <div style={{ background: 'var(--accent-soft)', border: '1px solid var(--border)', borderRadius: '12px', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        {/* Crafted By */}
                        {item.craftedBy && (
                            <div style={{ background: 'var(--accent-soft)', padding: '10px 14px', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: '0.8rem', fontWeight: '900', color: 'var(--text-main)' }}>{item.craftedBy}</span>
                            </div>
                        )}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {(() => {
                                        if (item.id === 'NOOB_CHEST') {
                                            return (
                                                <>
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-main)' }}>200x T1 Food</span>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#4ade80' }}>100%</span>
                                                    </div>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-main)' }}>1x T1 Sword</span>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#4ade80' }}>100%</span>
                                                    </div>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-main)' }}>1x T1 Bow</span>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#4ade80' }}>100%</span>
                                                    </div>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-main)' }}>1x T1 Staff</span>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#4ade80' }}>100%</span>
                                                    </div>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        return (
                                            <>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-main)' }}>{shardName}</span>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    {rarityComparison.map(q => {
                                        ...
                                        return (
                                            <div key={q.id} style={{
                                                background: isCurrent ? 'var(--accent-soft)' : 'var(--slot-bg)',
                                                padding: '8px 12px', borderRadius: '10px',
                                                border: `1px solid ${isCurrent ? q.color + '40' : 'var(--border)'}`,
