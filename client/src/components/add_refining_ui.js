...
const insertPoint = /\{selectedBuilding === 'GATHERING' && \([\s\S]+?\s+\)\}/g;
const match = content.match(insertPoint);

if (match) {
    const lastMatch = match[match.length - 1];
    content = content.replace(lastMatch, lastMatch + refiningSection);
    console.log("Refining section added.");
} else {
    console.log("Could not find Gathering section to append Refining section.");
}
