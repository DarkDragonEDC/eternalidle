        ...
        Object.entries(inventory).forEach(([itemId, qty]) => {
            ...
            if (slot.startsWith('rune_')) {
                ...
                if (item.type === 'RUNE') {
                    ...
                    if (itemMatch) {
                        const runeKey = itemMatch[1]; // e.g. WOOD_COPY or ORE_DUPLIC
                        ...
                        const normalizedTargetAct = targetAct === 'TOOL' ? 'TOOLS' : targetAct;
                        const normalizedTargetEff = targetEff === 'COPY' ? 'DUPLIC' : targetEff;

                        if (runeAct === normalizedTargetAct && runeEff === normalizedTargetEff) {
                            matches = true;
                        }
                    }
                }
                        ...
                        <div>
                            ...
                            <div
                                onClick={() => {
                                    ...
                                    if (bestCandidate.type === 'FOOD' && onEquipFood) {
                                        onEquipFood(bestCandidate);
                                    } else {
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ fontSize: '0.75rem', color: item.rarityColor || '#888' }}>
                                                        {item.type === 'RUNE' ? `Tier ${item.tier} • ${item.stars} Stars` :
                                                            item.type === 'FOOD' ? `Tier ${item.tier} • Heal ${item.healPercent}%` :
