...
// 1. Identify the GATHERING section start
const gatheringStart = content.indexOf("{selectedBuilding === 'GATHERING' && (");
if (gatheringStart === -1) {
    console.log("Could not find GATHERING start");
    process.exit(1);
}

// 2. Identify the REFINING section start
const refiningStart = content.indexOf("{selectedBuilding === 'REFINING' && (");
if (refiningStart === -1) {
    console.log("Could not find REFINING start");
    process.exit(1);
}

// 3. Identify the BANK section start (the next major sibling)
const bankStart = content.indexOf("{selectedBuilding === 'BANK' && (");
if (bankStart === -1) {
    console.log("Could not find BANK start");
    process.exit(1);
}

// We want to reconstruct the area from gatheringStart to bankStart.
// The original plan was:
// {selectedBuilding === 'GATHERING' && ( ... )}
// {selectedBuilding === 'REFINING' && ( ... )}

// Let's grab the GATHERING content (before the misplaced REFINING block)
const gatheringHeader = `                            {selectedBuilding === 'GATHERING' && (
...
// Replace everything between gatheringStart and bankStart with our clean version.
const newContent = content.substring(0, gatheringStart) + gatheringHeader + content.substring(bankStart);

fs.writeFileSync('GuildPanel.jsx', newContent);
console.log("GuildPanel.jsx fixed and cleaned.");
