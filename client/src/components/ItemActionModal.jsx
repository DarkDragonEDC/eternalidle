...
const ItemActionModal = ({ item: rawItem, onClose, onEquip, onEquipFood, onSell, onList, onUse, onDismantle, onDeposit, onWithdraw, customAction, isIronman }) => {
    ...
    return createPortal(
        <AnimatePresence>
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 30000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
                    ...
                    style={{
                        ...
                        backdropFilter: 'blur(20px)', maxHeight: '90vh'
                    }}>
                    ...
                    <div style={{ padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }} className="custom-scrollbar">
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            ...
                            {onDeposit && (
                                <ActionButton onClick={() => { onDeposit(item.id); onClose(); }} icon={Package} label="Deposit to Bank" />
                            )}
                            {onWithdraw && (
