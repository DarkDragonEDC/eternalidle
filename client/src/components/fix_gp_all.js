...
// 1. Fix GUILD_HALL block (it was corrupted)
// Find the block starting from {/* GUILD_HALL Section */} to {/* GATHERING_STATION Section */}
...
// 2. Update GATHERING, REFINING, CRAFTING loops
const buildings = ['GATHERING_STATION', 'REFINING_STATION', 'CRAFTING_STATION'];
buildings.forEach(bId => {
    const searchStr = \`const silverCost = GUILD_BUILDINGS.\${bId}.baseSilverCost + (currentLevel * GUILD_BUILDINGS.\${bId}.perLevelSilverCost);\`;
    if (content.includes(searchStr)) {
        content = content.replace(searchStr, \`const silverCost = GUILD_BUILDINGS.\${bId}.baseSilverCost + (currentLevel * GUILD_BUILDINGS.\${bId}.perLevelSilverCost);
                                            const gpCost = GUILD_BUILDINGS.\${bId}.baseGPCost + (currentLevel * GUILD_BUILDINGS.\${bId}.perLevelGPCost);\`);
        
        content = content.replace(\`const hasSilver = (guild.bank_silver || 0) >= silverCost;\`, \`const hasSilver = (guild.bank_silver || 0) >= silverCost;
                                            const hasGP = (guild.guild_points || 0) >= gpCost;\`);
        
        content = content.replace(\`const canUpgrade = playerHasPermission('manage_upgrades') && hasGuildLevel && hasSilver && hasMats && !isSyncBlocked;\`, \`const canUpgrade = playerHasPermission('manage_upgrades') && hasGuildLevel && hasSilver && hasGP && hasMats && !isSyncBlocked;\`);
        
        // Add GP Requirement Card in the UI
        const trophyCard = \`<div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(0,0,0,0.2)', padding: '6px', borderRadius: '8px' }}>
        ...
        const gpCard = \`<div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(0,0,0,0.2)', padding: '6px', borderRadius: '8px' }}>
                                                                <ClipboardList size={12} color="var(--accent)" />
                                                                <span style={{ fontSize: '0.7rem', color: hasGP ? '#44ff44' : '#ff4444', fontWeight: 'bold' }}>{gpCost.toLocaleString()} GP</span>
                                                            </div>\`;
        
        if (content.includes(trophyCard)) {
           content = content.replace(trophyCard, trophyCard + "\\n" + gpCard);
        }
        
        console.log(\`Updated \${bId} logic\`);
    } else {
        console.log(\`Could not find search string for \${bId}\`);
    }
});
