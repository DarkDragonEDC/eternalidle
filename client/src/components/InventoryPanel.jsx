...
const InventoryPanel = ({ gameState, socket, settings, onEquip, onListOnMarket, onShowInfo, onUse, isMobile, onSelectItem, isPreviewActive, onPreviewActionBlocked }) => {
    ...
    const [activeTab, setActiveTab] = useState('INVENTORY'); // INVENTORY or BANK
    ...
    const confirmEquipFood = () => {
        ...
        onEquip(foodEquipModal.item.id, qty);
    ...
    const handleDismantle = (itemId) => {
        ...
        const entry = gameState?.state?.inventory?.[itemId];
        const qty = (entry && typeof entry === 'object') ? (entry.amount || 0) : (Number(entry) || 0);
        const quality = (entry && typeof entry === 'object') ? (entry.quality || 0) : 0;
    ...
    const handleDeposit = (itemId) => {
        if (isPreviewActive) return onPreviewActionBlocked();
        const item = resolveItem(itemId);
        if (!item) return;
        const entry = gameState?.state?.inventory?.[itemId];
        const qty = (entry && typeof entry === 'object') ? (entry.amount || 0) : (Number(entry) || 0);
        setDepositModal({ itemId, item, max: qty, quantity: qty });
    };
    ...
    const handleWithdraw = (itemId) => {
        ...
        const item = resolveItem(itemId);
        if (!item) return;
        const entry = gameState?.state?.bank?.items?.[itemId];
        const qty = (entry && typeof entry === 'object') ? (entry.amount || 0) : (Number(entry) || 0);
        setWithdrawModal({ itemId, item, max: qty, quantity: qty });
    };

    const handleBuySlot = () => {
        if (isPreviewActive) return onPreviewActionBlocked();
        const currentSlots = gameState?.state?.bank?.slots || 10;
        const cost = 10000 + (currentSlots - 10) * 5000;
        setBuySlotModal({ cost });
    };

    const confirmBuySlot = () => {
        if (isPreviewActive) return onPreviewActionBlocked();
        if (socket) socket.emit('buy_bank_slot');
        setBuySlotModal(null);
    };

    const rawItems = activeTab === 'INVENTORY'
    ...
    if (activeTab === 'INVENTORY') {
        ...
        totalSlots = baseSlots + extraSlots;
    } else {
        totalSlots = gameState?.state?.bank?.slots || 10;
    }
    ...
    // Prepend Buy Prompt for Bank if not at max capacity
    if (activeTab === 'BANK' && (gameState?.state?.bank?.slots || 10) < 50) {
        itemsToRender.unshift({ isBuyPrompt: true });
    }

    return (
        <div className="glass-panel" style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: isMobile ? '10px' : '20px', borderRadius: '16px', overflow: 'hidden' }}>
            {/* Tab Switcher */}
            <div style={{ display: 'flex', gap: '2px', background: 'var(--panel-bg)', borderRadius: '12px', padding: '4px', marginBottom: '15px', border: '1px solid var(--border)' }}>
                {['INVENTORY', 'BANK'].map(tab => (
                    <button
                        key={tab}
                        ...
                        style={{
                            ...
                            color: activeTab === tab ? 'var(--panel-bg)' : 'var(--text-dim)',
                            border: 'none',
                            fontWeight: '900',
                            fontSize: '0.75rem',
                            cursor: 'pointer',
                    ...
                    >
                        {tab === 'INVENTORY' ? <Package size={14} /> : <Coins size={14} />}
                        {tab}
                    </button>
                ))}
            </div>
            ...
            <div className="scroll-container" style={{ flex: 1, paddingRight: '5px', overflowY: 'auto' }}>
                <div style={{
                    ...
                    {itemsToRender.map((item, index) => {
                        // Special Rendering for Buy Prompt
                        if (item?.isBuyPrompt) {
                            const currentSlots = gameState?.state?.bank?.slots || 10;
                            const cost = 10000 + (currentSlots - 10) * 5000;
                            return (
                                ...
                                >
                                    <div style={{ color: 'var(--accent)', filter: 'drop-shadow(0 0 5px var(--accent))' }}>
                                        <PlusCircle size={24} />
                                    </div>
                                    <div style={{ fontSize: '0.65rem', fontWeight: 'bold', color: 'var(--text-main)', textAlign: 'center' }}>
                                        BUY SLOT
                                    </div>
                                    <div style={{ fontSize: '0.6rem', color: 'var(--accent)', fontWeight: '900' }}>
                                        {formatSilver(cost)}
                                    </div>
                                </div>
                            );
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <button
                                        ...
                                        style={{ width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--accent-soft)', border: '1px solid var(--border)', color: 'var(--text-main)', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
                                    >
                                        -
                                    </button>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button
                                    onClick={() => setSellModal(null)}
                                    style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-dim)', borderRadius: '8px', fontWeight: 'bold' }}
                                >
                                    CANCEL
                                </button>
                                <button
                ...
                usePotionModal && createPortal(
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ overflowY: 'auto', paddingRight: '4px', position: 'relative', zIndex: 1 }} className="custom-scrollbar">
                    ...
                    <div style={{
                        ...
                        <div style={{ overflowY: 'auto', paddingRight: '4px', position: 'relative', zIndex: 1 }} className="custom-scrollbar">
                            <div style={{ textAlign: 'center', marginBottom: '25px' }}>
                                ...
                                <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginTop: '4px' }}>
                                    {dismantleModal.item.name} (Available: <span style={{ color: 'var(--accent)', fontWeight: 'bold' }}>{dismantleModal.max}</span>)
                                </div>
                            </div>
                            ...
                            <div style={{ marginBottom: '25px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                                    <button
                                        ...
                                        style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#ccc', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                    >MIN</button>
                                    ...
                                    <button
                                        ...
                                        style={{ padding: '8px 12px', background: 'rgba(139, 92, 246, 0.1)', border: '1px solid rgba(139, 92, 246, 0.3)', color: '#a78bfa', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                    >MAX</button>
                                </div>
                                ...
                                <div style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '8px' }}>
                                        <span style={{ color: 'var(--text-main)', opacity: 0.7 }}>You will receive:</span>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', background: 'rgba(139, 92, 246, 0.15)', padding: '4px 10px', borderRadius: '8px', border: '1px solid rgba(139, 92, 246, 0.3)', minWidth: '40px' }}>
                                            <img src="/items/Rune_Shard.webp" alt="" style={{ width: '16px', height: '16px', objectFit: 'contain' }} />
                                            <span style={{ color: '#a78bfa', fontWeight: '900', fontSize: '0.9rem' }}>
                                                {(((dismantleModal.item.tier || 1) - 1) * 5 + (dismantleModal.quality || 0) + 1) * dismantleModal.quantity}
                                            </span>
                                        </div>
                                    </div>
                                    <div style={{ opacity: 0.6 }}>
                                        Dismantling grants shards and materials based on the item tier and rarity.
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '12px' }}>
                                ...
                                <button
                                    onClick={() => {
                                        if (isPreviewActive) return onPreviewActionBlocked();
                                        socket.emit('dismantle_item', { itemId: dismantleModal.itemId, quantity: parseInt(dismantleModal.quantity) });
                            ...
                            <div style={{ overflowY: 'auto', paddingRight: '4px', position: 'relative', zIndex: 1 }} className="custom-scrollbar">
                                <div style={{ textAlign: 'center', marginBottom: '25px' }}>
                                    <div style={{
                                        ...
                                        <img src={depositModal.item.icon} alt="" style={{ width: depositModal.item.scale || '40px', height: depositModal.item.scale || '40px', objectFit: 'contain' }} />
                                    </div>
                                    <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--text-main)', fontWeight: '900', letterSpacing: '1px' }}>
                                        Deposit to Bank
                                    </h3>
                                    <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginTop: '4px' }}>
                                        {depositModal.item.name} (Available: <span style={{ color: 'var(--accent)', fontWeight: 'bold' }}>{depositModal.max}</span>)
                                    </div>
                                </div>
                                ...
                                <div style={{ marginBottom: '25px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                                        <button
                                            onClick={() => setDepositModal({ ...depositModal, quantity: 1 })}
                                            style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#ccc', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                        >MIN</button>

                                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', background: 'var(--slot-bg)', borderRadius: '12px', border: '1px solid var(--border)', padding: '4px' }}>
                                            <button
                                                onClick={() => setDepositModal({ ...depositModal, quantity: Math.max(1, depositModal.quantity - 1) })}
                                                style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', fontSize: '1.2rem' }}
                                            >-</button>
                                            <input
                                                type="number"
                                            ...
                                            />
                                            <button
                                                onClick={() => setDepositModal({ ...depositModal, quantity: Math.min(depositModal.max, depositModal.quantity + 1) })}
                                                style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', fontSize: '1.2rem' }}
                                            >+</button>
                                        </div>

                                        <button
                                            onClick={() => setDepositModal({ ...depositModal, quantity: depositModal.max })}
                                            style={{ padding: '8px 12px', background: 'rgba(144, 213, 255, 0.1)', border: '1px solid rgba(144, 213, 255, 0.3)', color: 'var(--accent)', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                        >MAX</button>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            if (isPreviewActive) return onPreviewActionBlocked();
                                            socket.emit('deposit_to_bank', { itemId: depositModal.itemId, quantity: parseInt(depositModal.quantity) });
                            ...
                            <div style={{ overflowY: 'auto', paddingRight: '4px', position: 'relative', zIndex: 1 }} className="custom-scrollbar">
                                <div style={{ textAlign: 'center', marginBottom: '25px' }}>
                                    <div style={{
                                        ...
                                        <img src={withdrawModal.item.icon} alt="" style={{ width: withdrawModal.item.scale || '40px', height: withdrawModal.item.scale || '40px', objectFit: 'contain' }} />
                                    </div>
                                    <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--text-main)', fontWeight: '900', letterSpacing: '1px' }}>
                                        Withdraw from Bank
                                    </h3>
                                    <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginTop: '4px' }}>
                                        {withdrawModal.item.name} (In Bank: <span style={{ color: 'var(--accent)', fontWeight: 'bold' }}>{withdrawModal.max}</span>)
                                    </div>
                                </div>
                                ...
                                <div style={{ marginBottom: '25px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                                        <button
                                            onClick={() => setWithdrawModal({ ...withdrawModal, quantity: 1 })}
                                            style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#ccc', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                        >MIN</button>

                                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', background: 'var(--slot-bg)', borderRadius: '12px', border: '1px solid var(--border)', padding: '4px' }}>
                                            <button
                                                onClick={() => setWithdrawModal({ ...withdrawModal, quantity: Math.max(1, withdrawModal.quantity - 1) })}
                                                style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', fontSize: '1.2rem' }}
                                            >-</button>
                                            <input
                                                type="number"
                                            ...
                                            />
                                            <button
                                                onClick={() => setWithdrawModal({ ...withdrawModal, quantity: Math.min(withdrawModal.max, withdrawModal.quantity + 1) })}
                                                style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', fontSize: '1.2rem' }}
                                            >+</button>
                                        </div>

                                        <button
                                            onClick={() => setWithdrawModal({ ...withdrawModal, quantity: withdrawModal.max })}
                                            style={{ padding: '8px 12px', background: 'rgba(144, 213, 255, 0.1)', border: '1px solid rgba(144, 213, 255, 0.3)', color: 'var(--accent)', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.7rem' }}
                                        >MAX</button>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            if (isPreviewActive) return onPreviewActionBlocked();
                                            socket.emit('withdraw_from_bank', { itemId: withdrawModal.itemId, quantity: parseInt(withdrawModal.quantity) });
            ...
            {/* Buy Slot Confirmation Modal */}
            {buySlotModal && createPortal(
                <div style={{
                    position: 'fixed',
                    top: 0, left: 0, right: 0, bottom: 0,
                    ...
                    <div style={{
                        ...
                        <h3 style={{ margin: '0 0 10px 0', fontSize: '1.4rem', color: 'var(--text-main)', fontWeight: '900' }}>Confirm Purchase</h3>
                        <p style={{ margin: '0 0 25px 0', fontSize: '0.9rem', color: 'var(--text-dim)', lineHeight: '1.5' }}>
                            Are you sure you want to buy an additional bank slot for <span style={{ color: 'var(--accent)', fontWeight: 'bold' }}>{formatSilver(buySlotModal.cost)}</span>?
                        </p>
                        ...
                        <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: '900',
                                    fontSize: '0.9rem',
                                    boxShadow: '0 4px 15px rgba(144,213,255,0.3)'
                                }}
                            >
                                CONFIRM
                            </button>
