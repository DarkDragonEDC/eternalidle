...
export const isBetterItem = (candidate, current) => {
    ...
    // 1. Efficiency (Priority for Tools) - Tools follow efficiency first, then tier
        ...
        if (slot.startsWith('rune_')) {
            ...
            if (item.type === 'RUNE') {
                ...
                if (itemMatch) {
                    ...
                    // Normalize candidate for legacy support
                    if (runeEff === 'COPY') runeEff = 'DUPLIC';
                    if (runeEff === 'SPEED') runeEff = 'AUTO';
