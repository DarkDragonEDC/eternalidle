...
export const BASE_QUALITY_CHANCES = {
    1: { q4: 0.70, q3: 4.90, q2: 7.20, q1: 15.00 },
    2: { q4: 0.625, q3: 4.38, q2: 6.65, q1: 14.445 },
    3: { q4: 0.55, q3: 3.86, q2: 6.10, q1: 13.89 },
    4: { q4: 0.475, q3: 3.34, q2: 5.55, q1: 13.335 },
    5: { q4: 0.40, q3: 2.82, q2: 5.00, q1: 12.78 },
    6: { q4: 0.325, q3: 2.305, q2: 4.45, q1: 12.22 },
    7: { q4: 0.25, q3: 1.785, q2: 3.90, q1: 11.665 },
    8: { q4: 0.175, q3: 1.265, q2: 3.35, q1: 11.11 },
    9: { q4: 0.10, q3: 0.745, q2: 2.80, q1: 10.555 },
    10: { q4: 0.025, q3: 0.225, q2: 2.25, q1: 10.00 }
};
...
const GATHER_DATA = {
    xp: [0.5, 1, 2, 3.5, 5.5, 8, 11, 14.5, 18.5, 23],
    time: [8, 10, 15, 20, 25, 30, 35, 40, 45, 50]
};
const REFINE_DATA = {
    xp: [1, 2, 4, 7, 11, 16, 22, 29, 37, 46],
    time: [8, 10, 15, 20, 25, 30, 35, 40, 45, 50]
};
const POTION_XP_CURVE = [5, 10, 20, 35, 55, 80, 110, 145, 185, 230];
const CRAFT_DATA = {
    xp: [20, 40, 80, 140, 220, 320, 440, 580, 740, 920],
    time: [120, 160, 240, 320, 400, 480, 560, 640, 720, 760]
};
...
const POTION_TYPES = {
    ...
    CRIT: { name: 'Critical Potion', suffix: '_POTION_CRIT', desc: 'Increases Critical Chance', scale: 0.01, base: 0.00 },
    DAMAGE: { name: 'Damage Potion', suffix: '_POTION_DAMAGE', desc: 'Increases Damage', scale: 0.01, base: 0.00 }
};
...
const genPotions = () => {
    for (const [key, data] of Object.entries(POTION_TYPES)) {
        ...
        for (const t of TIERS) {
            ...
            else if (key === 'CRIT') val = POTION_SCALING.CRIT[t - 1] / 100;
            else if (key === 'DAMAGE') val = POTION_SCALING.DAMAGE[t - 1] / 100;
            ...
            const potionItem = {
                ...
                icon: {
                    ...
                    'SILVER': `/items/T${t}_SILVER_POTION.webp`,
                    'QUALITY': `/items/T${t}_QUALITY_POTION.webp`,
                    'GATHER_XP': `/items/T${t}_GATHERING_POTION.webp`,
                    'CRAFT_XP': `/items/T${t}_CRAFTING_POTION.webp`,
                    'DROP': `/items/T${t}_LUCK_POTION.webp`,
                    'GLOBAL_XP': `/items/T${t}_KNOWLEDGE_POTION.webp`,
                    'DAMAGE': `/items/T${t}_DAMAGE_POTION.webp`,
                    'CRIT': `/items/T${t}_CRITICAL_POTION.webp`
                }[key] || null,
                req: {
                    [`T${t}_EXTRACT`]: 5
                },
...
const ENHANCEMENT_STONE_DEFS = [
    ...
    { key: 'WARRIOR_OFFHAND', id: 'ENHANCEMENT_STONE_WARRIOR_OFFHAND', name: 'Warrior offhand enhancement stone', targetClass: 'WARRIOR', targetSlot: 'OFF_HAND' },
    ...
    { key: 'HUNTER_OFFHAND', id: 'ENHANCEMENT_STONE_HUNTER_OFFHAND', name: 'Hunter offhand enhancement stone', targetClass: 'HUNTER', targetSlot: 'OFF_HAND' },
    ...
    { key: 'MAGE_OFFHAND', id: 'ENHANCEMENT_STONE_MAGE_OFFHAND', name: 'Mage offhand enhancement stone', targetClass: 'MAGE', targetSlot: 'OFF_HAND' },
...
const genGear = (category, slot, type, idSuffix, matType, statMultipliers = {}) => {
    for (const t of TIERS) {
        ...
        // Cost Scaling - FIXED to 10 for all tiers/slots as requested (previously 20)
        mainMatCount = 10;

        if (Array.isArray(matType)) {
            matType.forEach(m => {
                req[`T${t}_${m}`] = mainMatCount;
            });
        } else if (matType === 'POUCH_PLANK') {
            req[`T${t}_PLANK`] = mainMatCount * 2; // Results in 20
        } else {
...
const genWarriorGear = (slot, type, idSuffix, matType, lookupName) => {
    for (const t of TIERS) {
        let req = {};
        const count = (type === 'WEAPON' || type === 'OFF_HAND') ? 10 : 20;

        if (Array.isArray(matType)) {
            matType.forEach(m => req[`T${t}_${m}`] = count);
        } else {
            req[`T${t}_${matType}`] = count;
        }
...
// --- WARRIOR GEAR ---
genWarriorGear('SWORD', 'WEAPON', 'SWORD', ['BAR', 'LEATHER'], 'Sword');
genWarriorGear('SHEATH', 'OFF_HAND', 'SHEATH', ['BAR', 'LEATHER'], 'Sheath');
...
genHunterGear('BOW', 'WEAPON', 'BOW', ['PLANK', 'LEATHER'], 'Bow');
genHunterGear('TORCH', 'OFF_HAND', 'TORCH', ['PLANK', 'LEATHER'], 'Torch');
...
genMageGear('FIRE_STAFF', 'WEAPON', 'FIRE_STAFF', ['PLANK', 'CLOTH'], 'Fire Staff');
genMageGear('TOME', 'OFF_HAND', 'TOME', ['PLANK', 'CLOTH'], 'Tome');
...
// --- TOOLMAKER ---
genGear('TOOLMAKER', 'PICKAXE', 'TOOL_PICKAXE', 'PICKAXE', ['BAR', 'PLANK'], { eff: 1 });
genGear('TOOLMAKER', 'AXE', 'TOOL_AXE', 'AXE', ['BAR', 'PLANK'], { eff: 1 });
genGear('TOOLMAKER', 'SKINNING_KNIFE', 'TOOL_KNIFE', 'SKINNING_KNIFE', ['BAR', 'LEATHER'], { eff: 1 });
genGear('TOOLMAKER', 'SICKLE', 'TOOL_SICKLE', 'SICKLE', ['BAR', 'CLOTH'], { eff: 1 });
genGear('TOOLMAKER', 'FISHING_ROD', 'TOOL_ROD', 'FISHING_ROD', ['BAR', 'PLANK'], { eff: 1 });
genGear('TOOLMAKER', 'POUCH', 'TOOL_POUCH', 'POUCH', 'POUCH_PLANK', { eff: 1 });
...
// --- NAME OVERRIDE: Pouch -> Basket ---
for (const t of TIERS) {
    if (ITEMS.GEAR.TOOLMAKER.POUCH[t]) {
        ITEMS.GEAR.TOOLMAKER.POUCH[t].name = 'Basket';
        ITEMS.GEAR.TOOLMAKER.POUCH[t].description = `A Tier ${t} Basket. Equip to increase stats.`;
    }
}
...
export const resolveItem = (itemOrId, overrideQuality = null) => {
    ...
    // Legacy Support (13/02/26): Alias COPY -> DUPLIC, SPEED -> AUTO and TOOL -> TOOLS for runes
    if (baseId.includes('_RUNE_')) {
        if (baseId.includes('_COPY')) baseId = baseId.replace('_COPY', '_DUPLIC');
        if (baseId.includes('_SPEED') && !baseId.includes('ATTACK_SPEED')) baseId = baseId.replace('_SPEED', '_AUTO');
        // Carefully handle TOOL -> TOOLS without double-suffixing
        if (baseId.includes('_TOOL') && !baseId.includes('_TOOLS')) {
            baseId = baseId.replace('_TOOL', '_TOOLS');
        }
    }
    ...
    // 3.1 Rune Base ID Fallback (For Search/Buy Orders)
    if (!baseItem && upperId.includes('_RUNE_')) {
        // Many systems might use T1_RUNE_WOOD_XP as a base, which doesn't exist (only _1STAR, _2STAR, _3STAR)
        baseItem = ITEM_LOOKUP[`${upperId}_1STAR`];
        if (baseItem) {
            baseId = `${upperId}_1STAR`;
        }
    }
            ...
            for (const key in baseItem.stats) {
                if (typeof baseItem.stats[key] === 'number') {
                    if (key === 'efficiency' && baseItem.isTool) {
                        ...
                        newStats[key] = parseFloat(((1.0 + (index * (39 / 26.5))) * 0.65).toFixed(1));
                    } else if (key === 'speed') {
    ...
    // Ensure the ID returned is normalized if it's a rune
    let finalId = itemId;
    if (itemId.includes('_RUNE_')) {
        const upper = itemId.toUpperCase();
        if (upper.includes('_COPY')) finalId = finalId.replace(/_COPY/gi, '_DUPLIC');
        if (upper.includes('_SPEED') && !upper.includes('ATTACK_SPEED')) finalId = finalId.replace(/_SPEED/gi, '_AUTO');
        if (upper.includes('_TOOL_')) finalId = finalId.replace(/_TOOL_/gi, '_TOOLS_');
    }

    const finalItem = {
        ...baseItem,
        id: finalId,
        name: baseItem.name?.replace(/_/g, ' '),
...
export const calculateItemSellPrice = (item, itemId) => {
    ...
    // 1. Raw Resources (Gathering materials like Wood, Ore, etc.)
    const rawResourcePrices = { 1: 1, 2: 2, 3: 4, 4: 7, 5: 11, 6: 16, 7: 22, 8: 29, 9: 37, 10: 46 };

    // 2. Refined Resources / Food / Maps (Basic flat pricing per tier)
    const refinedBasePrices = { 1: 2, 2: 4, 3: 8, 4: 14, 5: 22, 6: 32, 7: 44, 8: 58, 9: 74, 10: 92 };

    // 3. Potions
    const potionPrices = { 1: 10, 2: 20, 3: 40, 4: 70, 5: 110, 6: 160, 7: 220, 8: 290, 9: 370, 10: 475 };

    // 4. Equipment Base Prices (Base price = COMMON price)
    const equipmentBasePrices = { 1: 40, 2: 80, 3: 160, 4: 280, 5: 440, 6: 640, 7: 880, 8: 1160, 9: 1480, 10: 1840 };

    // 5. Legacy Equipment Table (Still used for Runes, Shards, Crests)
    const legacyEquipmentPrices = {
    ...
    if (item.type === 'RAW' || item.type === 'RESOURCE') {
    ...
    if (item.type === 'REFINED') {
        ...
        if (isRefined) {
            return refinedBasePrices[tier] || 2;
        }
        return rawResourcePrices[tier] || 1;
    }

    if (item.type === 'POTION') {
        return potionPrices[tier] || 10;
    }

    if (item.type === 'FOOD' || item.type === 'MAP') {
        return refinedBasePrices[tier] || 2;
    }

    // 6. Special Logic for Runes, Shards, and Crests (Legacy Table)
    if (item.type === 'RUNE' || item.type === 'CRAFTING_MATERIAL' || (item.type === 'RESOURCE' && item.id?.includes('SHARD'))) {
        const legacyTable = legacyEquipmentPrices[tier] || legacyEquipmentPrices[1];
        return legacyTable[rarity] || legacyTable['COMMON'] || 88;
    }

    // 7. Equipment / Weapons (New base + quality multiplier)
    if (equipmentBasePrices[tier]) {
        const basePrice = equipmentBasePrices[tier];

        // Detect quality from ID suffix (e.g. _Q1) or the quality property
        let qualityLevel = null;

        if (item.quality !== undefined && item.quality > 0) {
            qualityLevel = String(item.quality);
        } else {
            const suffix = item.id?.split('_Q')?.[1];
            if (suffix) {
                const qValue = parseInt(suffix);
                if (!isNaN(qValue)) qualityLevel = String(qValue);
            }
        }

        // Multipliers based on CSV inspection:
        // Common (No Q, Quality 0): 1.0x
        // Good (Q1, Quality 1): 2.0x base
        // Outstanding (Q2, Quality 2): 3.0x base
        // Excellent (Q3, Quality 3): 1.5x base
        // Masterpiece (Q4, Quality 4): 2.5x base
        const multipliers = {
            '1': 2.0, // Good
            '2': 3.0, // Outstanding
            '3': 1.5, // Excellent
        ...
        if (qualityLevel && multipliers[qualityLevel]) {
            return basePrice * multipliers[qualityLevel];
        }
        return basePrice;
    }
...
export const calculateRuneBonus = (tier, stars, effType = null) => {
    ...
    // Combat runes (ATTACK, SAVE_FOOD, BURST) keep their specific logic.
...
for (const t of TIERS) {
    ...
    ALL_RUNE_TYPES.forEach(typeKey => {
        ...
        for (let s = 1; s <= 3; s++) {
            ...
            const EFF_NAME_MAP = { XP: 'XP', DUPLIC: 'Duplication', SPEED: 'Auto-Refine', AUTO: 'Auto-Refine', EFF: 'Efficiency', ATTACK: 'Attack', SAVE_FOOD: 'Food Saving', BURST: 'Burst', ATTACK_SPEED: 'Attack Speed' };
            const EFF_LABEL_MAP = { XP: 'Experience', DUPLIC: 'Duplication', SPEED: 'Auto-Refine Chance', AUTO: 'Auto-Refine Chance', EFF: 'Speed', ATTACK: 'Damage', SAVE_FOOD: 'Conservation', BURST: 'Critical Chance', ATTACK_SPEED: 'Attack Speed' };

            const actName = ACT_NAME_MAP[act] || act;
            let effName = EFF_NAME_MAP[eff] || eff;
            let effectLabel = EFF_LABEL_MAP[eff] || eff;

            // SPECIAL CASE: Fishing SPEED/AUTO rune is "Auto-Cooking"
...
export const formatItemId = (itemOrId, options = {}) => {
    ...
    // Special handling for Enhancement Stones: ENHANCEMENT_STONE_HUNTER_BOOTS -> Hunter boots enhancement stone
    if (baseId.startsWith('ENHANCEMENT_STONE_')) {
        const parts = baseId.replace('ENHANCEMENT_STONE_', '').split('_');
        const className = parts[0].charAt(0).toUpperCase() + parts[0].slice(1).toLowerCase();
        const slotName = parts.slice(1).join(' ').toLowerCase();
        return `${className} ${slotName} enhancement stone`;
    }
