...
// All tables referenced in the project server code
const ALL_TABLES = [
  'accounts',
  'ban_list',
  'characters',
...
async function main() {
  ...
  for (const table of ALL_TABLES) {
    ...
    if (r1.exists && !r2.exists) {
      console.log('\n[MISSING] TABLE "' + table + '": EXISTS in Project, MISSING in New');
      if (r1.columns && r1.columns.length > 0) console.log('   Project columns: [' + r1.columns.join(', ') + ']');
      hasIssues = true;
      continue;
    }
    if (!r1.exists && r2.exists) {
      console.log('\n[EXTRA] TABLE "' + table + '": MISSING in Project, EXISTS in New');
      if (r2.columns && r2.columns.length > 0) console.log('   New columns: [' + r2.columns.join(', ') + ']');
      hasIssues = true;
      continue;
    }
    ...
    if (r1.empty && r2.empty) {
      console.log('\n[WARN] TABLE "' + table + '": Both EMPTY (columns unknown)');
      continue;
    }
    if (r1.empty) {
      console.log('\n[WARN] TABLE "' + table + '": Project EMPTY, New has ' + r2.columns.length + ' columns: [' + r2.columns.join(', ') + ']');
      continue;
    }
    if (r2.empty) {
      console.log('\n[WARN] TABLE "' + table + '": New EMPTY, Project has ' + r1.columns.length + ' columns: [' + r1.columns.join(', ') + ']');
      continue;
    }

    const cols1 = r1.columns;
    const cols2 = r2.columns;
    const onlyIn1 = cols1.filter(function(c) { return !cols2.includes(c); });
    const onlyIn2 = cols2.filter(function(c) { return !cols1.includes(c); });

    if (onlyIn1.length === 0 && onlyIn2.length === 0 && cols1.length === cols2.length) {
      console.log('\n[OK] TABLE "' + table + '": IDENTICAL (' + cols1.length + ' columns)');
    } else {
      hasIssues = true;
      console.log('\n[DIFF] TABLE "' + table + '": COLUMNS DIFFER');
