...
for (const cls of CLASSES) {
    for (let t = 1; t <= 10; t++) {
        for (let q = 0; q <= 4; q++) {
            ...
            // BURST (Critical Strike) runes: ((tier-1)*5 + starBonus) * 0.3
            const starBonusMap = { 1: 1, 2: 3, 3: 5 };
            let baseBurst = (runeTier - 1) * 5 + (starBonusMap[runeStars] || runeStars);
            let burstRuneBonus = Math.max(0.5, parseFloat((baseBurst * 0.3).toFixed(1)));
