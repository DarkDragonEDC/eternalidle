    ...
    async processCombatRound(char, currentTime = null) {
        ...
        if (combat.mobHealth <= 0) {
            ...
            try {
                ...
                let finalXp = baseXp * (1 + xpBonus / 100); // +1% per point

                // --- FARM CAP: Proficiency Overleveling Penalty ---
                const mobLevel = mobData?.level || (currentTier <= 1 ? 1 : (currentTier - 1) * 10);
                ...
                // --- FARM CAP REMOVED ---
                // No longer applying penalties to XP/Silver. 100% rewards.
                const farmCapPenalty = 1.0;

                finalXp = finalXp * farmCapPenalty;
                ...
                let finalSilver = 0;
                if (mobData && mobData.silver && Math.random() < 0.8) {
                ...
                if (mobData && mobData.loot) {
                    ...
                    for (let [lootId, lootInfo] of Object.entries(mobData.loot)) {
                        let chance = 0;
                        let minQty = 1;
                        let maxQty = 1;

                        if (Array.isArray(lootInfo)) {
                            // Format: [chance, minQty, maxQty] or [chance, qty]
                            chance = lootInfo[0];
                            minQty = lootInfo[1] || 1;
                            maxQty = lootInfo[2] || minQty;
                        } else {
                            // Legacy format: chance (number)
                            chance = lootInfo;
                        }
                        ...
                        if (Math.random() <= (chance * dropMult)) {
                            let dropQty = 1;
                            if (maxQty > minQty) {
                                dropQty = Math.floor(Math.random() * (maxQty - minQty + 1)) + minQty;
                            } else {
                                dropQty = minQty;
                            }

                            this.gameManager.inventoryManager.addItemToInventory(char, lootId, dropQty);
                            roundDetails.lootGained.push(`${dropQty}x ${lootId}`);
                            message += ` [Item: ${dropQty}x ${lootId}]`;

                            // Accumulate Session Loot
                            if (!combat.sessionLoot) combat.sessionLoot = {};
                            combat.sessionLoot[lootId] = (combat.sessionLoot[lootId] || 0) + dropQty;
                        }
                    }
                }
