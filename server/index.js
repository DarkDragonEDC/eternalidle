        ...
        try {
            await gameManager.executeLocked(userId, async () => {
                ...
                if (packageId.startsWith('MEMBERSHIP')) {
                    ...
                    const added = gameManager.inventoryManager.addItemToInventory(char, 'MEMBERSHIP', qty);
                    if (added) {
                        result = { success: true, message: `${qty}x Membership item${qty > 1 ? 's' : ''} added to inventory!` };
                    } else {
                        ...
                        result = { success: true, message: `Inventory full! ${qty}x Membership item${qty > 1 ? 's' : ''} sent to Market -> Claims tab.` };
                    }
                ...
                if (result.success) {
                    ...
                    userSockets.forEach(s => {
                        ...
                        gameManager.getStatus(userId, false, characterId).then(status => {
...
app.get('/api/leaderboard', async (req, res) => {
    try {
        ...
        res.json({ type: response.type, mode: response.mode, data: response.top100 });
    } catch (err) {
...
io.on('connection', (socket) => {
    ...
    console.log(`[VERSION] Emitting server version: ${pkg.version} to match client ${pkg.version}`);
    ...
    if (socket.user?.id) {
        ...
        }).catch(err => console.error('[SOCKET] Error checking initial ban status:', err));
    }
    ...
    socket.on('acknowledge_ban_warning', async () => {
        ...
        gameManager.broadcastToUser(socket.user.id, 'account_status', { banWarning: null });
    });
    ...
    socket.on('join_character', async ({ characterId }) => {
        ...
        try {
            ...
            if (ban && (ban.level === 2 || ban.level === 3)) {
                ...
                console.log(`[BAN] Blocking login for ${socket.user.email} (Level ${ban.level})`);
                return;
            }
            ...
            await gameManager.executeLocked(userId, async () => {
                ...
                if (status.state && status.state.tutorialStep && status.state.tutorialStep !== 'COMPLETED') {
                    console.log(`[TUTORIAL] Auto-completing character ${characterId} (was at step: ${status.state.tutorialStep})`);
                ...
                if (cachedChar && cachedChar.offlineReport) {
                    ...
                    console.log(`[OFFLINE] Including offline report in join_character status_update`);
                }
                ...
                try {
                    const canSpin = await gameManager.dailyRewardManager.canSpin(cachedChar);
    ...
    socket.on('change_avatar', async ({ avatar }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                const isFree = freeAvatars.some(fa => normalizedAvatar.includes(normalizeAvatar(fa)));
                const isUnlocked = unlocked.some(ua => normalizedAvatar.includes(normalizeAvatar(ua)));

                if (!isFree && !isUnlocked) {
                    throw new Error("This avatar is locked! Unlock it first for 200 Orbs.");
                }
    ...
    socket.on('change_banner', async ({ banner }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (!isFree && !isUnlocked) {
                    throw new Error("This banner is locked! Unlock it first for 200 Orbs.");
                }
    ...
    socket.on('unlock_banner', async ({ bannerName }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                socket.emit('status_update', await gameManager.getStatus(socket.user.id, true, socket.data.characterId));
                socket.emit('action_result', { success: true, message: `Unlocked ${bannerName} banner!` });
            });
        } catch (err) {
    ...
    socket.on('unlock_avatar', async ({ avatarName }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (char.state.unlockedAvatars.some(ua => normalizeAvatar(ua) === normalizedAvatarName)) {
                    throw new Error("Avatar already unlocked!");
                }

                const premiumAvatars = ['5 - male', '5 - female'];
                const isPremiumAvatar = premiumAvatars.some(pa => normalizedAvatarName.includes(pa));
                ...
                socket.emit('status_update', await gameManager.getStatus(socket.user.id, true, socket.data.characterId));
                socket.emit('action_result', { success: true, message: `Unlocked ${avatarName} avatar!` });
            });
        } catch (err) {
    ...
    socket.on('unlock_theme', async ({ themeId }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                socket.emit('action_result', { success: true, message: `Unlocked ${themeId} theme!` });
                socket.emit('status_update', await gameManager.getStatus(socket.user.id, true, socket.data.characterId));
            });
        } catch (err) {
            socket.emit('error', { message: err.message });
    ...
    socket.on('get_public_profile', async ({ characterName }) => {
        try {
            ...
            socket.emit('error', { message: err.message || String(err), stack: err.stack, fullError: err });
        }
    });
    ...
    socket.on('rest_heal', async ({ percent }) => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                const userId = socket.user.id;
                const char = await gameManager.getCharacter(userId, socket.data.characterId);
                if (!char) return socket.emit('action_result', { restHealError: 'Character not found.' });

                // Validate not in combat or dungeon
                if (char.state?.combat) {
                    return socket.emit('action_result', { restHealError: 'Cannot rest during combat!' });
                }
                if (char.state?.dungeon?.active) {
                    return socket.emit('action_result', { restHealError: 'Cannot rest during a dungeon!' });
                }

                // Already resting?
                if (char.state?.resting) {
                    return socket.emit('action_result', { restHealError: 'Already resting!' });
                }

                // Validate percent
                const validPercents = [10, 25, 50, 75, 100];
                if (!validPercents.includes(percent)) {
                    return socket.emit('action_result', { restHealError: 'Invalid heal percentage.' });
                }
                ...
                if (missingHp <= 0) {
                    return socket.emit('action_result', { restHealError: 'You are already at full health!' });
                }
                ...
                if (healAmount <= 0) {
                    return socket.emit('action_result', { restHealError: 'No healing needed for this amount.' });
                }
                ...
                if (silver < cost) {
                    return socket.emit('action_result', { restHealError: 'Not enough silver!' });
                }
                ...
                console.log(`[REST] ${char.name} started resting: heal ${healAmount} HP for ${cost} silver (${percent}%), duration ${durationSeconds}s`);

                socket.emit('action_result', { restHealStarted: true, healAmount, cost, durationSeconds });
                const status = await gameManager.getStatus(userId, true, socket.data.characterId);
                socket.emit('status_update', status);
            });
        } catch (err) {
    ...
    socket.on('cancel_rest', async () => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                console.log(`[REST] ${char.name} cancelled resting, refunded ${char.state.resting.cost} silver`);
    ...
    socket.on('rest_check', async () => {
        try {
            if (!socket.data.characterId || socket.data.characterId === 'undefined') return;
            const userId = socket.user.id;
            const status = await gameManager.getStatus(userId, false, socket.data.characterId);
            socket.emit('status_update', status);
        } catch (err) {
    ...
    function checkEquipmentLock(char, itemType, itemId) {
        ...
        if (isFarming) {
            ...
            if (itemType === 'RUNE' && itemId) {
                if (itemId.includes('_GATHERING_') || itemId.includes('_REFINING_') || itemId.includes('_TOOLS_')) {
    ...
    function checkEquipmentLockBySlot(char, slot) {
        ...
        if (isFarming) {
            ...
            if (slot.startsWith('rune_GATHERING_') || slot.startsWith('rune_REFINING_') || slot.startsWith('rune_TOOLS_')) {
                return 'Cannot change farm runes during an activity! Stop the activity first.';
            }
        }
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (char) {
                    ...
                    if (item) {
                        ...
                        if (lockError) {
                            socket.emit('action_result', { success: false, message: lockError });
                            return;
                        }
                    }
                }
        ...
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (char && (char.state?.combat || char.state?.dungeon || char.current_activity)) {
                    socket.emit('action_result', { success: false, message: 'Cannot switch equipment sets during an activity!' });
                    return;
                }
    ...
    socket.on('deposit_to_bank', async ({ itemId, quantity = 1 }) => {
        try {
            console.log(`[BANK] Deposit request: charId=${socket.data.characterId}, itemId=${itemId}, qty=${quantity}`);
            if (!socket.data.characterId || socket.data.characterId === 'undefined') return;
            await gameManager.executeLocked(socket.user.id, async () => {
                const char = await gameManager.getCharacter(socket.user.id, socket.data.characterId);
                if (!char) throw new Error("Character not found");
                ...
                console.log(`[BANK] Depositing ${qty} of ${itemId}. Available: ${availableQty}`);
                ...
                if (isNewSlot && usedBankSlots >= (bank.slots || 10)) {
                    socket.emit('action_result', { success: false, message: `Bank is full! (${usedBankSlots}/${bank.slots || 10} slots)` });
                    return;
                }
                ...
                gameManager.markDirty(char.id);
                const status = await gameManager.getStatus(socket.user.id, false, socket.data.characterId);
                console.log(`[BANK] Deposit success. Inventory items: ${Object.keys(char.state.inventory).length}, Bank items: ${Object.keys(char.state.bank.items).length}`);
    ...
    socket.on('withdraw_from_bank', async ({ itemId, quantity = 1 }) => {
        try {
            console.log(`[BANK] Withdraw request: charId=${socket.data.characterId}, itemId=${itemId}, qty=${quantity}`);
            if (!socket.data.characterId || socket.data.characterId === 'undefined') return;
            await gameManager.executeLocked(socket.user.id, async () => {
                const char = await gameManager.getCharacter(socket.user.id, socket.data.characterId);
                if (!char) throw new Error("Character not found");
                ...
                console.log(`[BANK] Withdrawing ${qty} of ${itemId}. Available in bank: ${bankQty}`);
                ...
                if (isNewInvSlot && itemData && !itemData.noInventorySpace) {
                    if (invManager.getUsedSlots(char) >= invManager.getMaxSlots(char)) {
                        socket.emit('action_result', { success: false, message: 'Inventory is full!' });
                        return;
                    }
                }
                ...
                gameManager.markDirty(char.id);
                const status = await gameManager.getStatus(socket.user.id, false, socket.data.characterId);
                console.log(`[BANK] Withdraw success. Inventory items: ${Object.keys(char.state.inventory).length}, Bank items: ${Object.keys(char.state.bank.items).length}`);
    ...
    socket.on('buy_bank_slot', async () => {
        try {
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (currentSlots >= 50) {
                    socket.emit('action_result', { success: false, message: 'Bank is already at maximum capacity (50 slots)!' });
                    return;
                }
                ...
                if (silver < cost) {
                    socket.emit('action_result', { success: false, message: `Not enough Silver! Need ${formatNumber(cost)} (you have ${formatNumber(silver)})` });
                    return;
                }
        ...
        try {
            ...
            if (listings && listings.length > 0) {
                ...
                if (filtered.length > 0) {
                    const unitPrices = filtered.map(l => Math.floor(l.price / l.amount));
            ...
            if (buyOrders && buyOrders.length > 0) {
                const filtered = buyOrders.filter(o => {
                    if (quality !== undefined && o.item_data?.quality !== quality) return false;
            ...
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                if (char) {
                    ...
                    if (inCombat && type?.toUpperCase() === 'COMBAT') {
                        socket.emit('action_result', { success: false, message: 'Cannot change combat runes during combat!' });
                        return;
                    }
                    if (isFarming && ['GATHERING', 'REFINING', 'TOOLS'].includes(type?.toUpperCase())) {
                        socket.emit('action_result', { success: false, message: 'Cannot change farm runes during an activity!' });
                        return;
                    }
                }
        ...
        try {
            ...
            const results = data.map(char => {
                ...
                return {
                    ...
                    isIronman: !!(char.state?.isIronman || char.name?.toLowerCase() === 'ironman' || char.name?.toLowerCase().startsWith('[im]'))
                };
            });
    ...
    socket.on('trade_cancel', async ({ tradeId }) => {
        try {
            await gameManager.executeLocked(socket.user.id, async () => {
                ...
                Array.from(connectedSockets.values())
                    .filter(s => affectedIds.includes(s.data.characterId))
                    .forEach(s => s.emit('trade_update', { id: tradeId, status: 'CANCELLED' }));
            });
        } catch (err) {
            socket.emit('error', { message: err.message });
        }
    ...
    try {
        ...
        await Promise.all(Object.values(charGroups).map(async ({ user, charId, sockets }) => {
            try {
                ...
                const key = `${user.id}:${charId}`;
                if (!tickCounters.has(key)) tickCounters.set(key, { count: 0, lastFullSync: 0 });
                ...
                await gameManager.executeLocked(user.id, async () => {
                    ...
                    const isActive = isInCombat || isInDungeon || isInWorldBoss || isGathering;
                    ...
                    if (result) {
                        ...
                        const needsFullSync = (counter.count - counter.lastFullSync >= 10)
                            || result.leveledUp
