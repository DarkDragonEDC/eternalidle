...
for (let t = 1; t <= 10; t++) {
    ...
    // Find damage that causes ~10 food consumption
    let low = 1;
    let high = 5000;
    let bestDmg = 0;
    let bestFoodCount = 0;
    ...
    results.push({
        tier: t,
        mobDmg: bestDmg,
        foodCount: bestFoodCount,
        maxHP: player.maxHP,
        totalHeal: bestFoodCount * healPerUse
    });
}
...
function simulate(tier, player, mobDmgBase, healPerUse) {
    ...
    for (const mobId of mobIds) {
        ...
        while (mHealth > 0 && currentHp > 0 && timeElapsed < 300000) {
        ...
        if (currentHp <= 0) return 999; // Death
...
console.log("| Tier | Mob DMG | Food Items | Total Heal | Player Max HP |");
console.log("|------|---------|------------|------------|---------------|");
results.forEach(r => {
    console.log(`| ${r.tier} | ${r.mobDmg} | ${r.foodCount.toFixed(1)} | ${r.totalHeal} | ${r.maxHP} |`);
});
